...
while True:
    (rval, im) = webcam.read()
    im = cv2.resize(im, (320, 220))
    #im = cv2.flip(im, 1, 0)  # Flip to act as a mirror
    # Resize the image to speed up detection
    mini = cv2.resize(im, (int(im.shape[1] / size), int(im.shape[0] / size)))    # detect MultiScale / faces
    ...
    if time.time() > future:##after 20second music will play
        try:
            ...
            if text == 'Angry':
                randomfile = random.choice(os.listdir("D:/Documents/1-Music/Songs/Angry/"))
                print('You are angry !!!! please calm down:) ,I will play song for you :' + randomfile)
                file = ('D:/Documents/1-Music/Songs/Angry/' + randomfile)
                subprocess.call([mp, file])

            if text == 'Happy':
                randomfile = random.choice(os.listdir("D:/Documents/1-Music/Songs/Happy/"))
                print('You are smiling :) ,I playing special song for you: ' + randomfile)
                file = ('D:/Documents/1-Music/Songs/Happy/' + randomfile)
                subprocess.call([mp, file])

            if text == 'Mask':
                #randomfile = random.choice(os.listdir(""))
                print('Please remove mask to Detect emotions ')
                #file = ('D:/Documents/1-Music/Songs/mask/' + randomfile)
                #subprocess.call([mp, file])

            if text == 'Sad':
                randomfile = random.choice(os.listdir("D:/Documents/1-Music/Songs/Sad/"))
                print('You are sad,dont worry:) ,I playing song for you: ' + randomfile)
                file = ('D:/Documents/1-Music/Songs/Sad/' + randomfile)
                subprocess.call([mp, file])
            break
